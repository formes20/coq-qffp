...
type command =
  ...
  | CDefineSort of symbol * symbol list * sort
...
let string_of_command c =
  ...
  | CDefineSort (sn, sargs, ssort) -> "(define-sort " ^ string_of_symbol sn
                                     ^ " (" ^ String.concat " " (List.map string_of_symbol sargs) ^ ") "
                                     ^ string_of_sort ssort ^ ")"
...
let pp_command out c =
  ...
  | CDefineSort (sn, sargs, ssort) ->
    output_string out "(define-sort ";
    output_string out (string_of_symbol sn);
    output_string out " (";
    output_string out (String.concat " " (List.map string_of_symbol sargs));
    output_string out ") ";
    output_string out (string_of_sort ssort);
    output_string out ")";
...
let fn_fp_isZero = "fp.isZero"
let fn_fp_isNormal = "fp.isNormal"
let fn_fp_isSubNormal = "fp.isSubnormal"
let fn_fp_isInfinite = "fp.isInfinite"
let fn_fp_isNaN = "fp.isNaN"
let fn_fp_isPositive = "fp.isPositive"
let fn_fp_isNegative = "fp.isNegative"

let fn_fp_eq = "fp.eq"
let fn_fp_lt = "fp.lt"
let fn_fp_gt = "fp.gt"
let fn_fp_leq = "fp.leq"
let fn_fp_geq = "fp.geq"

let fn_fp_abs = "fp.abs"
let fn_fp_neg = "fp.neg"

let fn_fp_max = "fp.max"
let fn_fp_min = "fp.min"

let fn_fp_mul = "fp.mul"
let fn_fp_add = "fp.add"
let fn_fp_sub = "fp.sub"
let fn_fp_fma = "fp.fma"
let fn_fp_div = "fp.div"
let fn_fp_sqrt = "fp.sqrt"
let fn_fp_rem = "fp.rem"
let fn_fp_roundToIntegral = "fp.roundToIntegral"

let fn_to_fp = "to_fp"
let fn_to_fp_unsigned = "to_fp_unsigned"
let fn_fp_to_ubv = "fp.to_ubv"
let fn_fp_to_sbv = "fp.to_sbv"
...
let rec ttyp_of_sort s sm: ttyp =
  match s with
  | SIdentifier (ISimple s) when s = bool_type_name -> TBool
  | SIdentifier (IIndexed (s, [INumeral n])) when s = "BitVec" -> TBitVec (Z.to_int n)
  | SIdentifier (IIndexed (s, (INumeral eb)::(INumeral sb)::[])) when s = "FloatingPoint" -> TFloatVec ((Z.to_int eb), (Z.to_int sb))
  | SIdentifier (ISimple s) when s = "Float16" -> TFloatVec (5, 11)
  | SIdentifier (ISimple s) when s = "Float32" -> TFloatVec (8, 24)
  | SIdentifier (ISimple s) when s = "Float64" -> TFloatVec (11, 53)
  | SIdentifier (ISimple s) when s = "Float128" -> TFloatVec (15, 113)
  | SIdentifier (ISimple s) when s = "RoundingMode" -> TRoundingMode
  | SIdentifier (ISimple s) -> 
    begin
      try
        let ssort = M.find s sm in
          ttyp_of_sort ssort sm
      with Not_found ->
        failwith ("Undefined sort: " ^ s)
    end
...
let ttyp_of_identifier tm i : ttyp =
  ...
  | ISimple s ->
     begin
       if s = "true" || s = "false"
       then TBool
       else if s = "RNE" || s = "RNA" || s = "RTP" || s = "RTN" || s = "RTZ" ||
              s = "roundNearestTiesToEven" || s = "roundNearestTiesToAway" ||
              s = "roundTowardPositive" || s = "roundTowardNegative" ||
              s = "roundTowardZero"
            then TRoundingMode
       else
         try
  ...
  | IIndexed (s, is) ->
     ...
     else if  Str.string_match (Str.regexp "+oo\\|-oo\\|+zero\\|-zero\\|NaN") s 0 
     then match is with
          | (INumeral eb)::(INumeral sb)::[] -> TFloatVec ((Z.to_int eb), (Z.to_int sb))
          | _ -> failwith ("Failed to determine the type of " ^ string_of_identifier i)
     else failwith ("Failed to determine the type of " ^ string_of_identifier i)

let ttyp_of_qual_identifier tm sm qi : ttyp =
  match qi with
  | QIdentifier i -> ttyp_of_identifier tm i
  | QAnnotated (_, s) -> ttyp_of_sort s sm

let rec ttyp_of_term (tm : tm) (fm : fm) (sm : sm) (t : term) : ttyp =
  let fail () = failwith ("Failed to determine the type of " ^ string_of_term t) in
  match t with
  | TConstant sc -> ttyp_of_spec_constant sc
  | TVariable qi -> ttyp_of_qual_identifier tm sm qi
  | TApplication (qi, ts) -> let fn = symbol_of_qual_identifier qi in
                             begin
                               ...
                               | QIdentifier (ISimple v), _::a1::a2::[] when v = fn_ite -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_concat -> (match ttyp_of_term tm fm sm a1, ttyp_of_term tm fm sm a2 with
                               ...
                               | QIdentifier (ISimple v), a1::[] when v = fn_bvnot -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvand -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvor -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::[] when v = fn_bvneg -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::_ when v = fn_bvadd -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvmul -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvudiv -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvurem -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvshl -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvlshr -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvult -> TBool
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvnand -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvnor -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvxor -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvxnor -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvcomp -> TBitVec 1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvsub -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvsdiv -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvsrem -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvsmod -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_bvashr -> ttyp_of_term tm fm sm a1
                               | QIdentifier (IIndexed (v, [INumeral n])), a1::[] when v = fn_repeat -> (match ttyp_of_term tm fm sm a1 with
                                                                                                         | TBitVec m -> TBitVec (Z.to_int n * m)
                                                                                                         | _ -> fail())
                               | QIdentifier (IIndexed (v, [INumeral n])), a1::[] when v = fn_zero_extend -> (match ttyp_of_term tm fm sm a1 with
                                                                                                              | TBitVec m -> TBitVec (Z.to_int n + m)
                                                                                                              | _ -> fail())
                               | QIdentifier (IIndexed (v, [INumeral n])), a1::[] when v = fn_sign_extend -> (match ttyp_of_term tm fm sm a1 with
                                                                                                              | TBitVec m -> TBitVec (Z.to_int n + m)
                                                                                                              | _ -> fail())
                               | QIdentifier (IIndexed (v, [INumeral n])), a1::[] when v = fn_rotate_left -> ttyp_of_term tm fm sm a1
                               | QIdentifier (IIndexed (v, [INumeral n])), a1::[] when v = fn_rotate_right -> ttyp_of_term tm fm sm a1
                               ...
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isZero -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isNormal -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isSubNormal -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isInfinite -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isNaN -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isPositive -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_isNegative -> TBool
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_eq -> TBool
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_lt -> TBool
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_gt -> TBool
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_leq -> TBool
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_geq -> TBool
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_abs -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::[] when v = fn_fp_neg -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_max -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_min -> ttyp_of_term tm fm sm a1
                               | QIdentifier (ISimple v), a1::a2::a3::[] when v = fn_fp_mul -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::a3::[] when v = fn_fp_add -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::a3::[] when v = fn_fp_sub -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::a3::a4::[] when v = fn_fp_fma -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::a3::[] when v = fn_fp_div -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_sqrt -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_rem -> ttyp_of_term tm fm sm a2
                               | QIdentifier (ISimple v), a1::a2::[] when v = fn_fp_roundToIntegral -> ttyp_of_term tm fm sm a2
                               | QIdentifier (IIndexed (v, (INumeral eb)::(INumeral sb)::[])), a1::[] when v = fn_to_fp -> TFloatVec ((Z.to_int eb), (Z.to_int sb))
                               | QIdentifier (IIndexed (v, (INumeral eb)::(INumeral sb)::[])), a1::a2::[] when v = fn_to_fp -> TFloatVec ((Z.to_int eb), (Z.to_int sb))
                               | QIdentifier (IIndexed (v, (INumeral eb)::(INumeral sb)::[])), a1::a2::[] when v = fn_to_fp_unsigned -> TFloatVec ((Z.to_int eb), (Z.to_int sb)) 
                               | QIdentifier (IIndexed (v, (INumeral n)::[])), a1::a2::[] when v = fn_fp_to_ubv -> TBitVec (Z.to_int n)
                               | QIdentifier (IIndexed (v, (INumeral n)::[])), a1::a2::[] when v = fn_fp_to_sbv -> TBitVec (Z.to_int n)
                               | QIdentifier (ISimple v), a1::a2::a3::[] when v = "fp" -> let ty_s = ttyp_of_term tm fm sm a1 in
  ...
  | TLet (vbs, t') ->
     let tm' = List.fold_left (
                   fun tm (s, t) ->
                   M.add s (ttyp_of_term tm fm sm t) tm
                 ) tm vbs in
     ttyp_of_term tm' fm sm t'

let is_bool_term tm fm sm t =
  match ttyp_of_term tm fm sm t with
  | TBool -> true
  | _ -> false
...
let rec convert_defined_extensions_term ?skip:(ops=SS.empty) tm fm sm term =
  match term with
  | TConstant _ -> term
  | TVariable _ -> term
  | TApplication (QIdentifier (ISimple v), ts)
       when v = fn_distinct && not (SS.mem v ops) -> let ts = List.rev (List.rev_map (convert_defined_extensions_term ~skip:ops tm fm sm) ts) in
  ...
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvnand && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                   let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                   make_bvnot (make_bvand s t)
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvnor && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_bvnot (make_bvor s t)
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvxor && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_bvor (make_bvand s (make_bvnot t)) (make_bvand (make_bvnot s) t)
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvxnor && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                   let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                   make_bvor (make_bvand s t) (make_bvand (make_bvnot s) (make_bvnot t))
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvcomp && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                   let res =
                                                     if m = 1
                                                   ...
                                                   convert_defined_extensions_term ~skip:ops tm fm sm res
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvsub && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_bvadd s (make_bvneg t)
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvsdiv && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                   let m_minus_one = Z.of_int (m - 1) in
                                                   let msb_s = "|__msb_s__|" in
                                                   let msb_t = "|__msb_t__|" in
                                                   let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                   let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
  ...
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvsrem && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                   let m_minus_one = Z.of_int (m - 1) in
                                                   let msb_s = "|__msb_s__|" in
                                                   let msb_t = "|__msb_t__|" in
                                                   let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
  ...
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvsmod && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                   let m_minus_one = Z.of_int (m - 1) in
                                                   let msb_s = "|__msb_s__|" in
                                                   let msb_t = "|__msb_t__|" in
  ...
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvule && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_or (make_bvult s t) (make_eq s t)
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvugt && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_bvult t s
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvuge && not (SS.mem v ops) -> let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_or (make_bvult t s) (make_eq s t)
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvslt && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                  let m_minus_one = Z.of_int (m - 1) in
                                                  let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                  let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
                                                  make_or (make_and (make_eq (make_extract m_minus_one m_minus_one s) bin1)
                                                    ...
                                                    (make_and (make_eq (make_extract m_minus_one m_minus_one s) (make_extract m_minus_one m_minus_one t))
                                                       (make_bvult s t))
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvsle && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                  let m_minus_one = Z.of_int (m - 1) in
                                                  let res =
                                                    make_or (make_and (make_eq (make_extract m_minus_one m_minus_one s) bin1)
                                                               (make_eq (make_extract m_minus_one m_minus_one t) bin0))
  ...
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvsge && not (SS.mem v ops) -> let res = make_bvsle t s in
                                                  convert_defined_extensions_term ~skip:ops tm fm sm res
  | TApplication (QIdentifier (ISimple v), s::t::[])
       when v = fn_bvashr && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm s) in
                                                   let m_minus_one = Z.of_int (m - 1) in
                                                   let s = convert_defined_extensions_term ~skip:ops tm fm sm s in
                                                   let t = convert_defined_extensions_term ~skip:ops tm fm sm t in
  ...
  | TApplication (QIdentifier (IIndexed (v, [INumeral i])), t::[])
       when v = fn_zero_extend && not (SS.mem v ops) -> if Z.to_int i <= 0 then t
                                                             ...
                                                             convert_defined_extensions_term ~skip:ops tm fm sm res
  | TApplication (QIdentifier (IIndexed (v, [INumeral i])), t::[])
       when v = fn_sign_extend && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm t) in
                                                        let m_minus_one = Z.of_int (m - 1) in
                                                        if Z.to_int i <= 0 then t
                                                             ...
                                                             convert_defined_extensions_term ~skip:ops tm fm sm res
  | TApplication (QIdentifier (IIndexed (v, [INumeral i])), t::[])
       when v = fn_rotate_left && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm t) in
                                                        let m_minus_one = Z.of_int (m - 1) in
                                                        ...
                                                        if Z.to_int i <= 0 then t
                                                             ...
                                                             convert_defined_extensions_term ~skip:ops tm fm sm res
  | TApplication (QIdentifier (IIndexed (v, [INumeral i])), t::[])
       when v = fn_rotate_right && not (SS.mem v ops) -> let m = size_of_ttyp (ttyp_of_term tm fm sm t) in
                                                         let m_minus_one = Z.of_int (m - 1) in
                                                         if Z.to_int i <= 0 then t
                                                         else if m = 1 then t
                                                         else let res =
                                                              ...
                                                              convert_defined_extensions_term ~skip:ops tm fm sm res
  | TApplication (fn, ts) -> TApplication (fn, List.rev (List.rev_map (convert_defined_extensions_term ~skip:ops tm fm sm) ts))
  | TLet (vbs, t) -> let (tm', vbs_rev) =
                       List.fold_left (
                           fun (tm, vbs_rev) (v, vt) ->
                           let ty = ttyp_of_term tm fm sm vt in
                           let tm' = M.add v ty tm in
                           (tm', (v, convert_defined_extensions_term ~skip:ops tm fm sm vt)::vbs_rev)
                         ) (tm, []) vbs in
                     let t' = convert_defined_extensions_term ~skip:ops tm' fm sm t in
                     TLet (List.rev vbs_rev, t')

let convert_defined_extensions_command ?skip:(ops=SS.empty) tm fm sm c =
  match c with
  | CSetLogic _ -> (tm, fm, sm, c)
  | CSetInfo _ -> (tm, fm, sm, c)
  | CSetOption _ -> (tm, fm, sm, c)
  | CDefineSort (sn, sargs, ssort) ->
    begin
      match sargs with
      | [] -> (tm, fm, (M.add sn ssort sm), c)
  ...
  | CDeclareFun (fn, fargs, fsort) ->
     ...
     begin
       match fargs with
       | [] -> (M.add fn (ttyp_of_sort fsort sm) tm, fm, sm, c)
  ...
  | CDefineFun (fn, fargs, fsort, fterm) ->
     begin
       ...
       | [] -> let ty = ttyp_of_sort fsort sm in
               ...
               let fterm' = convert_defined_extensions_term ~skip:ops tm' fm sm fterm in
               (tm', fm, sm, CDefineFun (fn, fargs, fsort, fterm'))
       | _ -> let fm' = M.add fn (fargs, fsort, fterm) fm in
              let fterm' = convert_defined_extensions_term ~skip:ops tm fm' sm fterm in
              (tm, fm', sm, CDefineFun (fn, fargs, fsort, fterm'))
     end
  | CAssert t -> (tm, fm, sm, CAssert (convert_defined_extensions_term ~skip:ops tm fm sm t))
  | CCheckSat -> (tm, fm, sm, c)
  | CGetModel -> (tm, fm, sm, c)
  | CExit -> (tm, fm, sm, c)
  | CComment _ -> (tm, fm, sm, c)

let convert_defined_extensions_script ?skip:(ops=SS.empty) tm fm sm script =
  let (tm', fm', sm', cmds_rev) =
    List.fold_left
      (fun (tm, fm, sm, res) c ->
        let (tm', fm', sm', c') = convert_defined_extensions_command ~skip:ops tm fm sm c in
        (tm', fm', sm', c'::res)
      ) (tm, fm, sm, []) script in
  (tm', fm', sm', List.rev cmds_rev)
...
let convert_defined_extensions_file ?skip:(ops=[fn_distinct]) file =
  ...
  let fm = M.empty in
  let sm = M.empty in
  let (_, _, _, file') = convert_defined_extensions_script ~skip:ops tm fm sm file in
  file'
