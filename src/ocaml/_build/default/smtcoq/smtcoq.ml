...
let rec norm_ite_term g tm fm sm term =
  match term with
  | TConstant _ -> (g, [], term)
  | TVariable _ -> (g, [], term)
  | TApplication (QIdentifier (ISimple v), b::s::t::[]) when v = fn_ite -> let ty = ttyp_of_term tm fm sm s in
                                                                           begin
                                                                             match ty with
                                                                             | TBitVec n ->
                                                                                let (g1, cmds1, s') = norm_ite_term g tm fm sm s in
                                                                                let (g2, cmds2, t') = norm_ite_term g1 tm fm sm t in
  ...
  | TApplication (fn, fargs) -> let (g', cmds_rev, ts_rev) = List.fold_left (
                                                                 ...
                                                                 let (g', cmds', t') = norm_ite_term g tm fm sm t in
  ...
  | TLet (bs, t) -> let (g', tm', cmds_rev, bs_rev) = List.fold_left (
                                                          ...
                                                          let (g', cmds', t') = norm_ite_term g tm fm sm t in
                                                          let tm' = M.add v (ttyp_of_term tm fm sm t) tm in
...
let norm_ite_command g tm fm sm c =
  match c with
  | CSetLogic _ -> (g, tm, fm, sm, [], c)
  | CSetInfo _ -> (g, tm, fm, sm, [], c)
  | CSetOption _ -> (g, tm, fm, sm, [], c)
  | CDefineSort (sn, sargs, ssort) ->
    begin
      match sargs with
      | [] -> (g, tm, fm, (M.add sn ssort sm), [], c)
  ...
  | CDeclareFun (fn, fargs, fsort) ->
     ...
     begin
       match fargs with
       | [] -> (g, M.add fn (ttyp_of_sort fsort sm) tm, fm, sm, [], c)
  ...
  | CDefineFun (fn, fargs, fsort, fterm) ->
     begin
       ...
       | [] -> let ty = ttyp_of_sort fsort sm in
               ...
               let (g', cmds, fterm') = norm_ite_term g tm' fm sm fterm in
               (g', tm', fm, sm, cmds, CDefineFun (fn, fargs, fsort, fterm'))
       | _ -> let fm' = M.add fn (fargs, fsort, fterm) fm in
              let (g', cmds, fterm') = norm_ite_term g tm fm' sm fterm in
              (g', tm, fm', sm, cmds, CDefineFun (fn, fargs, fsort, fterm'))
     end
  | CAssert t -> let (g', cmds, t') = norm_ite_term g tm fm sm t in
                 (g', tm, fm, sm, cmds, CAssert t')
  | CCheckSat -> (g, tm, fm, sm, [], c)
  | CGetModel -> (g, tm, fm, sm, [], c)
  | CExit -> (g, tm, fm, sm, [], c)
  | CComment _ -> (g, tm, fm, sm, [], c)

let norm_ite_script g tm fm sm script =
  let (g', tm', fm', sm', cmds_rev) = List.fold_left (
                                     fun (g, tm, fm, sm, cmds_rev) c ->
                                     let (g', tm', fm', sm', cmds, c') = norm_ite_command g tm fm sm c in
                                     (g', tm', fm', sm', c'::(List.rev_append cmds cmds_rev))
                                   ) (g, tm, fm, sm, []) script in
  (g', tm', fm', sm', List.rev cmds_rev)
...
let norm_ite_file file =
  ...
  let fm = M.empty in
  let sm = M.empty in
  let (_, _, _, _, cmds) = norm_ite_script g tm fm sm file in
  cmds
...
let norm_name_command g tm c =
  ...
  | CSetOption _ -> (g, tm, c)
  | CDefineSort (sn, sargs, ssort) -> 
    let (g', sn') = gen_name g in
    let tm' = M.add sn sn' tm in
    (g', tm', CDefineSort (sn', sargs, ssort))
  | CDeclareFun (fn, fargs, fsort) ->
